class HomeController < ApplicationController
  ...
  def index
    ...
    if !logged_in?
      ...
      @past_shifts = Shift.for_past_days(7).chronological.by_store.by_employee.paginate(page: params[:ipage]).per_page(10)
      @upcoming_shifts = Shift.for_next_days(7).chronological.by_store.by_employee.paginate(page: params[:page]).per_page(10)
    elsif current_user.role == 'manager' and !@store.nil?
      @past_shifts = Shift.for_past_days(7).for_store(@store).chronological.paginate(page: params[:ipage]).per_page(10)
      ...
      @past_shifts = Shift.for_past_days(7).for_employee(@employee).chronological.paginate(page: params[:ipage]).per_page(10)
